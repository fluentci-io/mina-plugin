...
pub fn setup_mina() -> Result<String, Error> {
    ...
    let stdout = dag()
        ...
        .with_exec(vec!["gem install mina"])?
        .with_exec(vec!["[ -d $HOME/.local/bin ] || mkdir -p $HOME/.local/bin"])?
        .with_exec(vec![
            "ln -s `flox activate -- gem environment gemhome`/bin/mina $HOME/.local/bin/mina || true",
        ])?
        .with_exec(vec!["PATH=$HOME/.local/bin:$PATH", "type", "mina"])?
